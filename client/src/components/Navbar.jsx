    ...
    >
      <div className={`transition-all duration-500 ${
        ...
        <div className="max-w-7xl mx-auto px-4 md:px-6 py-3 flex items-center justify-between">
          ...
          {/* ── Logo group: CIT + divider + STM32 ── */}
          <div className="flex items-center gap-3">

            {/* CIT college logo — only the icon square part, white bg clipped */}
            <div className="flex-shrink-0 h-9 w-auto">
              <img
                src={citLogo}
                alt="Chennai Institute of Technology"
                className="h-full w-auto object-contain"
                /* The logo has a white background — use a dark rounded
                   container so it sits cleanly on the dark navbar.
                   On scroll (glass bg) it blends seamlessly. */
                style={{
                  borderRadius: 6,
            ...
            {/* vertical divider */}
            <div className="w-px h-7 bg-indigo-500/30 flex-shrink-0" />

            {/* STM32 brand */}
            <div className="flex items-center gap-2.5">
              <div className="relative isolate">
                <div className="w-8 h-8 bg-gradient-to-br from-indigo-500 to-violet-600 rounded-lg flex items-center justify-center glow-indigo">
              ...
              <div>
                ...
                <p className="font-display text-xs text-slate-400 leading-none tracking-wider mt-0.5">WORKSHOP</p>
              </div>
            </div>
          </div>
            ...
            <div className="px-6 py-4 flex flex-col gap-1">
              {NAV_LINKS.map((item, i) => (
                <motion.a
                  ...
                  href={`#${item.toLowerCase()}`}
                  onClick={e => { e.preventDefault(); handleNavLinkClick(`#${item.toLowerCase()}`); }}
